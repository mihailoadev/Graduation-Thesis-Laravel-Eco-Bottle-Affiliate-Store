                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    <img src="{{asset('uploads/products/' . $product->image)}}" style="width:24px; height: 24px; border-radius: 100px; margin-right:12px;" alt="">
                                    {{$product->name}}
                                </td>
                                <td>
                                    @php
                                ...
                                <td>
                                    @if($product->status == 1)
                                    <span class="badge badge-primary"><i class="fa-solid fa-bolt"></i> Active</span>
                                    @else
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group" aria-label="Product Actions">
                                        <button type="button" class="btn btn-dark btn-sm rounded me-1" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$product->id}}" data-name="{{$product->name}}" data-short_description="{{$product->short_description}}" data-description="{{$product->description}}" data-price="{{$product->price}}" data-image="{{$product->image}}" data-status="{{$product->status}}">
                                            <i class="fa fa-edit"> </i> View/Edit
                                        </button>
                                        <button class="btn btn-danger btn-sm rounded me-1 delete-button" onclick="deletePrompt({{$product->id}})" data-id="{{$product->id}}">
                                            <i class="fa fa-trash"> </i> Delete
                                        </button>
                                    </div>
                                    <form action="{{ route('admin.products-delete') }}" id="delForm-{{$product->id}}" method="post" style="display: none;">
                                        @csrf
