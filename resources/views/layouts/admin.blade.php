...
<head>
    ...
    @php
    $isSuperAdmin = Auth::guard('admin')->user()->is_SuperAdmin ?? null;
    @endphp
    <title>EcoBottle |
        @if ($isSuperAdmin === 1)
        Admin Dashboard
        @elseif ($isSuperAdmin === 0)
        Moderator Dashboard
        @else
        Unknown Role
        @endif
                ...
                <ul class="nav metismenu" id="side-menu">
                    <li class="nav-header">
                        <div class="dropdown profile-element">
                            <img alt="image" class="rounded-circle" style="width: 48px; height: 48px;" src="{{ asset('uploads/profile/' . (Auth::guard('admin')->user()->image ? Auth::guard('admin')->user()->image : (Auth::guard('admin')->user()->is_SuperAdmin ? 'administrator.jpg' : 'moderator.jpg'))) }}" />
                            <a data-toggle="dropdown" class="dropdown-toggle" href="#">
                                <span class="block m-t-xs font-bold">{{ Auth::guard('admin')->user()->name }}</span>
                                <span class="text-muted text-xs block">
                                    @php
                                    $isSuperAdmin = Auth::guard('admin')->user()->is_SuperAdmin;
                                    @endphp
                            ...
                            <ul class="dropdown-menu animated fadeInRight m-t-xs">
                                <li><a class="dropdown-item" href="profile"><i class="fa-solid fa-circle-user"></i> Profile</a></li>
                    ...
                    <li {{ request()->route()->getName() === 'admin.dashboard' ? "class=active" : '' }}>
                        <a href="{{route('admin.dashboard')}}"><i class="fa-solid fa-house"></i> <span class="nav-label">Dashboard</span></a>
                    </li>
                    @if(Auth::guard('admin')->check())
                    @if(Auth::guard('admin')->user()->is_SuperAdmin == 1)
                    <li {{ Str::contains(request()->path(), 'user') ? "class=active" : '' }}>
                        <a href="{{route('admin.users')}}"><i class="fa-solid fa-users"></i> <span class="nav-label">Users</span></a>
                    </li>
                    @endif
                    @endif
                    <li {{ request()->route()->getName() === 'admin.products' ? "class=active" : '' }}>
                        <a href="{{route('admin.products')}}"><i class="fa-solid fa-box-open"></i> <span class="nav-label">Products</span></a>
                    </li>
                    <li {{ request()->route()->getName() === 'admin.commission-levels' ? "class=active" : '' }}>
                        <a href="{{route('admin.commission-levels')}}"><i class="fa-solid fa-code-fork"></i> <span class="nav-label">Commssion Levels</span></a>
                    </li>
                    <li {{ request()->route()->getName() === 'admin.orders' ? "class=active" : '' }}>
                        <a href="{{route('admin.orders')}}"><i class="fa-solid fa-list-check"></i> <span class="nav-label">Orders</span></a>
                    </li>
                </ul>
            </div>
        </nav>
            ...
            <div class="row border-bottom">
                <nav class="navbar navbar-static-top white-bg" role="navigation" style="margin-bottom: 0">
                    ...
                    <ul class="nav navbar-top-links navbar-right">
                        <li>
                            <button type="button" class=" btn btn-primary mr-4" style="cursor: pointer; margin-right: 20px;" onclick="document.getElementById('logout-form').submit();">
                                <i class="fa-solid fa-arrow-right-from-bracket"></i> Logout</strong>
                            </button>
                        </li>
                    </ul>
                </nav>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="text-sm-end d-none d-sm-block">
                                Designed & Developed by Mihailo Anđelić
                            </div>
                        </div>
                    </div>
                </div>
